...
static gboolean draw_timeline(GtkWidget *widget, cairo_t *cr, gpointer user_data) {
    ...
    // Linhas de grade mais sutis e coloridas
    ...
    // Linhas de separação de tracks com cor mais vibrante
    ...
    while (iter != NULL) {
        ...
        // Cores vibrantes para os clips baseadas no track
        float hue = (track_num % 6) / 6.0f;
        float r, g, b;
        if (hue < 0.17f) { r = 0.2f; g = 0.6f; b = 0.9f; }      // Azul
        else if (hue < 0.33f) { r = 0.4f; g = 0.8f; b = 0.3f; }  // Verde
        else if (hue < 0.5f) { r = 0.9f; g = 0.5f; b = 0.2f; }   // Laranja
        else if (hue < 0.67f) { r = 0.9f; g = 0.3f; b = 0.6f; }  // Rosa
        else if (hue < 0.83f) { r = 0.7f; g = 0.4f; b = 0.9f; }  // Roxo
        else { r = 0.3f; g = 0.8f; b = 0.9f; }                    // Ciano
        
        // Gradiente para o fundo do clip
        ...
        // Texto do nome do arquivo com sombra
        ...
        if (read_wav_samples(clip->filename, &waveform_samples, &waveform_count, max_waveform_samples) == 0 && waveform_count > 0) {
            if (waveform_area_height > 15) {
                ...
                // Waveform com cor vibrante baseada no track
    ...
    // Cursor de reprodução com gradiente e brilho
